...
const BalloonsDisplay = ({
  ...
  <div className="flex justify-center gap-2 mb-4">
    <div className="w-10 h-10 flex items-center justify-center rounded-full bg-gray-700 text-gray-400 font-mono">
      1
    </div>
    {balloons.map((b, balloonIndex) => {
      const idx = balloonIndex + 1;
      const inRange =
        highlightRange && idx >= highlightRange.l && idx <= highlightRange.r;
    ...
    <div className="flex flex-col items-center gap-6">
      ...
      <div className="text-center">
        <div className="text-sm text-gray-500 mb-2">
          dp[l][r] = max coins bursting balloons l to r
        </div>
              ...
              <tr>
                ...
                {Array(n)
                  ...
                  .map((_, colIdx) => (
                    <th
                      key={`col-${colIdx + 1}`}
                      className="p-2 text-gray-400 w-12"
                    >
                      {colIdx + 1}
                    </th>
                  ))}
      ...
      {currentStep && (
        <div className="text-sm text-center">
          ...
          <div className="font-mono bg-gray-800/50 text-green-400 px-6 py-3 rounded-lg">
            {currentStep.formula}
          </div>
        </div>
      )}
      <div className="text-sm text-gray-500">
        Process by LENGTH: len=1, then len=2, ... Think: which balloon to burst
        LAST
      </div>
    </div>
  );
    ...
    <div className="bg-gray-800/30 rounded-lg p-6 max-w-lg">
      <div className="text-center mb-4">
        ...
        <div className="text-sm text-gray-400">
          Instead of which balloon to burst FIRST, think which to burst LAST.
        </div>
      </div>
      <div className="text-sm text-gray-400 space-y-2">
        <div>
          <span className="text-blue-400">dp[l][r]</span> = max coins from
          bursting balloons l to r
        </div>
        <div>
          If balloon k is burst <span className="text-red-400">LAST</span> in
          range [l, r]:
        </div>
        ...
        <div className="text-gray-500 text-xs mt-2">
          Process by length: smaller ranges first (they become subproblems)
        </div>
      </div>
    </div>
  </div>
  ...
  return (
    <div className="p-6 bg-gray-900 rounded-xl w-full max-w-4xl mx-auto">
      <div className="text-center mb-4">
        ...
        <div className="text-sm text-gray-400">
          Maximize coins by bursting balloons - think which to burst LAST
        </div>
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {(currentPhase === "table" || currentPhase === "backtrack") && (
