...
const buildCombinationSumTree = (): TreeNode => {
  const root: TreeNode = {
    id: "root",
    path: [],
    children: [],
  ...
  const build = (node: TreeNode, start: number) => {
    ...
    for (let i = start; i < CANDIDATES.length; i++) {
      const newRemaining = node.remaining - CANDIDATES[i];
      const child: TreeNode = {
        id: `${node.id}-${CANDIDATES[i]}-${node.children.length}`,
        path: [...node.path, CANDIDATES[i]],
        children: [],
        depth: node.depth + 1,
      ...
      if (newRemaining > 0) {
        build(child, i); // Pass i to allow reuse
      }
    }
  };
...
export default function CombinationSumVisualizer() {
  ...
  const tree = useMemo(() => buildCombinationSumTree(), []);
  ...
  const getMessage = () => {
    if (step === 0)
      return `Target = ${TARGET}. Find combinations from [${CANDIDATES.join(", ")}] that sum to ${TARGET}`;
    ...
    if (curr.status === "pruned") {
      return `Sum = ${curr.path.reduce((a, b) => a + b, 0)} > ${TARGET}. Prune this branch.`;
    }
  ...
  return (
    <div className="p-6 bg-gray-900 rounded-xl w-full max-w-4xl mx-auto">
      ...
      <div className="flex justify-center items-center gap-6 mb-6">
        <div className="flex gap-2">
          {CANDIDATES.map((num, idx) => (
            <motion.div
              key={idx}
              className="w-12 h-12 rounded-lg bg-indigo-600 flex items-center justify-center shadow-lg"
            >
        ...
        <motion.div
          className="w-14 h-14 rounded-lg bg-emerald-600 flex items-center justify-center shadow-lg"
          animate={{
            boxShadow:
      ...
      <div className="mt-4 pt-4 border-t border-gray-800 text-sm text-gray-500 text-center">
        <span className="text-emerald-400">Key:</span> Pass{" "}
        <code className="text-emerald-400">i</code> (not i+1) to allow reusing
        the same element
      </div>
    </div>
  );
