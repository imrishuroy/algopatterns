...
const MaskDisplay = ({
  ...
  <div className="flex justify-center gap-2 mb-4">
    {cities.map((city, cityIndex) => {
      const isVisited =
        visited.has(cityIndex) || (mask & (1 << cityIndex)) !== 0;
      return (
        <div key={`mask-city-${city}`} className="flex flex-col items-center">
          <div
            className={`w-12 h-12 flex items-center justify-center rounded-full font-mono font-bold text-lg transition-all ${
              isVisited
    ...
    <table className="border-collapse text-sm mx-auto">
      <thead>
        <tr>
          ...
          {cities.map((c) => (
            <th key={`city-${c}`} className="p-1.5 text-gray-400 w-10">
              {c}
            </th>
          ))}
        </tr>
        ...
        <table className="border-collapse text-xs mx-auto">
          <thead>
            <tr>
              ...
              {cities.map((c) => (
                <th key={`tsp-city-${c}`} className="p-1.5 text-gray-400 w-12">
                  {c}
                </th>
              ))}
            </tr>
                ...
                {cities.map((city, pos) => {
                  ...
                  return (
                    <td key={`dp-${mask}-${city}`} className="p-1">
                      <div
                        className={`w-12 h-8 flex items-center justify-center rounded font-mono text-xs ${
                          isCurrent
