#!/usr/bin/env bash
#
# sync_repo.sh - Create a git bundle and sync it to Sentinel
#
# Usage:
#   Full sync:        ./sync_repo.sh <repo_path> <api_repo_id> [base_ref]
#   Incremental sync: ./sync_repo.sh <repo_path> <api_repo_id> <base_ref>
#
# Requires SENTINEL_API_KEY environment variable.
#
# Creates a git bundle from a local repository and uploads it to Sentinel
...
: "${SENTINEL_API_KEY:?Set the SENTINEL_API_KEY environment variable}"

API_BASE="https://sentinel.deepsource.com/api/v1"
BUNDLE_FILE=$(mktemp /tmp/sentinel-bundle-XXXXXX.bundle)
