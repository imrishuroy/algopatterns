...
const wrongAnswer: Answer = {
  ...
  correctAnswer: 1,
  explanation:
    "O(1) is constant time. Binary search halves the search space each time, giving O(log n).",
};
...
describe("MultipleChoice", () => {
  it("renders question text", async () => {
    const MultipleChoice = (
      await import("@/components/quiz/questions/MultipleChoice")
    ).default;
    render(
      <MultipleChoice
...
describe("TrueFalse", () => {
  it("renders question text", async () => {
    const TrueFalse = (await import("@/components/quiz/questions/TrueFalse"))
      .default;
    render(
      <TrueFalse
  ...
  it("shows opacity-50 on non-selected non-correct option after answering", async () => {
    ...
    const nonSelectedBtns = Array.from(buttons).filter(
      (b) =>
        ...
        !b.className.includes("border-red-500")
    );
...
describe("ExplanationPanel", () => {
  it("shows Correct! when answer is correct", async () => {
    const ExplanationPanel = (
      await import("@/components/quiz/questions/ExplanationPanel")
    ).default;
  ...
  it("shows green styling when correct", async () => {
    ...
    const ExplanationPanelComponent = ExplanationPanel as React.FC<{
      answer: Answer;
    }>;
    const { container } = render(
      <ExplanationPanelComponent answer={correctAnswer} />
    );
  ...
  it("shows checkmark circle SVG when correct", async () => {
    ...
    const circleCheck = Array.from(svgs).find(
      (p) =>
        p.getAttribute("d") === "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"
    );
...
describe("ProblemsTab", () => {
  ...
  it("renders stats bar with total problem count", async () => {
    const ProblemsTab = (await import("@/app/patterns/[slug]/tabs/ProblemsTab"))
      .default;
    const { container } = render(
      <ProblemsTab
    ...
    const totalValue = container.querySelector(
      '[class*="text-white"][class*="font-bold"]'
    );
  ...
  it("links to problem detail with from param via Solve button", async () => {
    ...
    expect(solveLinks[0].closest("a")).toHaveAttribute(
      "href",
      "/problems/two-sum?from=arrays-strings"
    );
  });
  ...
  it("shows difficulty badges for each problem", async () => {
    ...
    const filtered = difficulties.filter((el) =>
  ...
  it("shows frequency for each problem", async () => {
    ...
    const freqs = screen.getAllByText((content) => content.startsWith("🔥"));
  ...
  it("sorts by name", async () => {
    ...
    expect(linkTexts[linkTexts.length - 1].textContent).toBe(
      "Valid Parentheses"
    );
  });
  ...
  it("calls onToggleComplete when checkbox is clicked", async () => {
    ...
    const toggleBtn = checkboxes.find(
      (b) =>
        b.className.includes("rounded-md") && b.className.includes("border-2")
    );
    if (toggleBtn) {
  ...
  it("shows +N for extra companies beyond 3", async () => {
    ...
    const hasPlusOne = Array.from(spans).some((s) =>
      s.textContent?.includes("+1")
    );
  ...
  it("has correct difficulty colors using CSS classes", async () => {
    ...
    const difficultySpans = container.querySelectorAll(
      '[class*="font-medium"]'
    );
