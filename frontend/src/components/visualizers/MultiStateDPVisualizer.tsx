...
const generateFeeSteps = (): FeeStep[] => {
  ...
  for (let i = 1; i < feePrices.length; i++) {
    ...
    if (newHold !== prevHold && newHold === prevCash - price) {
      ...
      newCash !== prevCash &&
      newCash === prevHold + price - transactionFee
    ) {
      decision = "sell";
    ...
    <div className="flex flex-col items-center gap-4">
        ...
        {productNums.map((num, idx) => (
          ...
          >
            ...
            <motion.div
              animate={{
                backgroundColor:
                  currentStep && idx === currentStep.index
                    ? "#a855f7"
                    : idx < (currentStep?.index ?? 0)
                      ? "#374151"
                      : "#1f2937",
            ...
            >
              <span className={num < 0 ? "text-red-400" : "text-white"}>
                {num}
              </span>
            </motion.div>
          </div>
      ...
      {currentStep && step > 0 && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className={`px-4 py-2 rounded-lg text-center font-medium ${decisionClass}`}
        >
          {decisionText}
        </motion.div>
      )}
      ...
      {step >= steps.length && step > 0 && (
        <div className="text-sm text-center bg-purple-600/20 px-4 py-2 rounded-lg">
          <span className="text-purple-400 font-bold">
            Answer: Maximum Product = {steps[steps.length - 1].maxProd}
          </span>
        </div>
      )}
      ...
      <div className="text-xs text-gray-500 text-center max-w-md">
        ...
        because today&apos;s minimum might become tomorrow&apos;s maximum!
      </div>
    </div>
  );
};
...
const CooldownPhase = ({
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <div className="text-sm text-gray-400">
        Prices: [{stockPrices.join(", ")}] | States: hold, sold, rest (1-day
        cooldown after sell)
      </div>
        ...
        {stockPrices.map((price, idx) => (
          ...
          >
            ...
            <motion.div
              animate={{
                backgroundColor:
                  currentStep && idx === currentStep.day
                    ? "#10b981"
                    : idx < (currentStep?.day ?? -1)
                      ? "#374151"
                      : "#1f2937",
      ...
      <svg width="400" height="180" className="mx-auto">
        ...
        <circle
          ...
          cy="50"
          r="35"
          fill={currentStep?.decision === "buy" ? "#2563eb" : "#1e3a5a"}
          stroke="#3b82f6"
          strokeWidth="2"
        />
        <text
        ...
        <circle
          ...
          cy="50"
          r="35"
          fill={currentStep?.decision === "sell" ? "#166534" : "#14532d"}
          stroke="#22c55e"
          strokeWidth="2"
        />
        <text
        ...
        <circle
          ...
          cy="140"
          r="35"
          fill={currentStep?.decision === "rest" ? "#854d0e" : "#713f12"}
          stroke="#eab308"
          strokeWidth="2"
        />
        <text
        ...
        />
        <text x="197" y="40" fill="#9ca3af" fontSize="9" textAnchor="middle">
          sell +price
        </text>
        ...
        <line
          ...
          markerEnd="url(#arrowhead)"
        />
        <text x="280" y="115" fill="#9ca3af" fontSize="9" textAnchor="middle">
          cooldown
        </text>
        ...
        <line
          ...
          markerEnd="url(#arrowhead)"
        />
        <text x="110" y="115" fill="#9ca3af" fontSize="9" textAnchor="middle">
          buy -price
        </text>

        <path
          d="M 65 50 A 40 40 0 0 1 100 15"
          stroke="#6b7280"
          strokeWidth="2"
          fill="none"
          markerEnd="url(#arrowhead)"
        />
        <text x="55" y="25" fill="#9ca3af" fontSize="9">
          keep
        </text>

        <path
          d="M 200 105 A 40 40 0 0 1 235 140"
          stroke="#6b7280"
          strokeWidth="2"
          fill="none"
          markerEnd="url(#arrowhead)"
        />
        <text x="245" y="155" fill="#9ca3af" fontSize="9">
          keep
        </text>
      </svg>
      ...
      {step >= steps.length && step > 0 && (
        <div className="text-sm text-center bg-green-600/20 px-4 py-2 rounded-lg">
          <span className="text-green-400 font-bold">
            Answer: Max Profit = $
            {Math.max(
              steps[steps.length - 1].sold === -9999
                ? -Infinity
                : steps[steps.length - 1].sold,
      ...
      <div className="text-xs text-gray-500 text-center max-w-md">
        State machine: After selling, must rest 1 day before buying again. hold
        = max(hold, rest - price), rest = max(rest, sold), sold = hold + price
      </div>
    </div>
  );
};
...
const FeePhase = ({ step, steps }: { step: number; steps: FeeStep[] }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <div className="text-sm text-gray-400">
        Prices: [{feePrices.join(", ")}] | Fee: ${transactionFee} per
        transaction
      </div>
      ...
      <div className="flex gap-4 text-sm">
        <div
          className={`px-3 py-1 rounded ${currentStep?.decision === "buy" ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-400"}`}
        >
          Buy: cash - price
        </div>
        <div
          className={`px-3 py-1 rounded ${currentStep?.decision === "sell" ? "bg-green-600 text-white" : "bg-gray-800 text-gray-400"}`}
        >
          Sell: hold + price - fee
        </div>
        <div
          className={`px-3 py-1 rounded ${currentStep?.decision === "hold" ? "bg-yellow-600 text-white" : "bg-gray-800 text-gray-400"}`}
        >
          Hold: no change
        </div>
      </div>
      ...
      {step >= steps.length && step > 0 && (
        <div className="text-sm text-center bg-green-600/20 px-4 py-2 rounded-lg">
          <span className="text-green-400 font-bold">
            Answer: Max Profit = ${steps[steps.length - 1].cash}
          </span>
        </div>
      )}

      <div className="text-xs text-gray-500 text-center max-w-md">
        Two states: hold and cash. Fee is paid when selling. hold = max(hold,
        cash - price), cash = max(cash, hold + price - fee)
      </div>
    </div>
  );
};
    ...
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="mt-4">
        <div className="text-xs text-gray-500 text-center mb-2">
          Min cost to paint houses 0..i ending with each color:
        </div>
        <div className="flex gap-4 justify-center">
          {colors.map((color, cIdx) => (
            ...
            >
              <div className={`text-xs ${colorClasses[cIdx].text} mb-1`}>
                {color}
              </div>
              <motion.div
      ...
      {step >= steps.length && step > 0 && (
        <div className="text-sm text-center bg-green-600/20 px-4 py-2 rounded-lg">
          ...
          <span className="text-gray-400 ml-2">
            ...
            {
              colors[
                steps[steps.length - 1].dp.indexOf(
                  Math.min(...steps[steps.length - 1].dp)
                )
              ]
            }{" "}
            for last house)
      ...
      <div className="text-xs text-gray-500 text-center max-w-md">
        dp[color] = cost[color] + min(dp[other colors]). No adjacent houses can
        have same color.
      </div>
    </div>
  );
  ...
  return (
    <div className="p-6 bg-gray-900 rounded-xl w-full max-w-5xl mx-auto">
      <div className="text-center mb-4">
        ...
        <div className="text-sm text-gray-400">
          Track multiple DP values at each position (when one value isn&apos;t
          enough)
        </div>
      </div>
          ...
          {phases.map((phase, index) => (
            ...
            >
              <span className="flex items-center gap-2">
                <span
                  className={`w-5 h-5 flex items-center justify-center rounded-full text-xs ${currentPhase === phase ? "bg-purple-500" : "bg-gray-700"}`}
                >
                  {index + 1}
                </span>
                {phaseLabels[phase]}
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {currentPhase === "product" && (
            <ProductPhase step={step} steps={productSteps} />
          )}
          {currentPhase === "cooldown" && (
            <CooldownPhase step={step} steps={cooldownSteps} />
          )}
          {currentPhase === "fee" && <FeePhase step={step} steps={feeSteps} />}
          {currentPhase === "paint" && (
            <PaintPhase step={step} steps={paintSteps} />
          )}
        </motion.div>
      </AnimatePresence>

      <div className="mt-6 pt-4 border-t border-gray-800 text-sm text-gray-500 text-center">
        Multi-State DP: When tracking ONE value per position isn&apos;t enough,
        track multiple states (max/min, hold/sold/rest, colors).
      </div>
    </div>
  );
}
