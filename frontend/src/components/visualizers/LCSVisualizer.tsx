  ...
  while (i > 0 && j > 0) {
    if (str1[i - 1] === str2[j - 1]) {
      ...
      steps.push({
        ...
        j,
        char: str1[i - 1],
        action: `Match '${str1[i - 1]}' - add to LCS, go diagonal`,
      });
      i--;
      j--;
      ...
      steps.push({
        i,
        j,
        action: `dp[${i - 1}][${j}] > dp[${i}][${j - 1}] - go up`,
      });
      i--;
    } else {
      steps.push({
        i,
        j,
        action: `dp[${i}][${j - 1}] >= dp[${i - 1}][${j}] - go left`,
      });
      j--;
    }
  }
...
const TablePhase = ({
  ...
  const currentDp = useMemo(() => {
    const table: number[][] = Array(str1.length + 1)
      .fill(null)
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      ...
      {currentStep && (
        <div
          className={`text-sm text-center font-mono px-6 py-3 rounded-lg ${currentStep.match ? "bg-green-600/20 text-green-400" : "bg-gray-800/50 text-gray-400"}`}
        >
          {currentStep.formula}
        </div>
      )}
      <div className="text-sm text-gray-500">
        If match: diagonal + 1 | Else: max(up, left)
      </div>
    </div>
  );
};

const BacktrackPhase = ({
  step,
  backtrackSteps,
  dp,
}: {
  step: number;
  backtrackSteps: { i: number; j: number; char?: string; action: string }[];
  dp: number[][];
}) => {
  const currentStep =
    step > 0 && step <= backtrackSteps.length ? backtrackSteps[step - 1] : null;
  const foundChars = backtrackSteps
    .slice(0, step)
    .filter((s) => s.char)
    .map((s) => s.char)
    .reverse();
...
export default function LCSVisualizer() {
  ...
  const { steps: backtrackSteps, lcs } = useMemo(
    () => generateBacktrackSteps(dp),
    [dp]
  );
