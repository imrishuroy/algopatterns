...
const generateTableSteps = () => {
  const n = nums.length;
  const steps: { i: number; j: number; newLen: number; formula: string; improved: boolean }[] = [];
...
const generateBinarySteps = () => {
  ...
  for (let i = 0; i < nums.length; i++) {
    ...
    if (left === tails.length) {
      tails.push(num);
      steps.push({ i, num, tails: [...tails], action: `${num} > all tails, append. LIS length = ${tails.length}` });
    } else {
      tails[left] = num;
      steps.push({ i, num, tails: [...tails], action: `Replace tails[${left}] with ${num}. LIS length = ${tails.length}` });
    }
  }
...
const ArrayDisplay = ({ highlightI, highlightJ }: { highlightI?: number; highlightJ?: number }) => (
  <div className="flex justify-center gap-1 mb-4">
    {nums.map((num, numIndex) => (
      <div key={`num-${num}-${numIndex}`} className="flex flex-col items-center">
...
const TablePhase = ({ step, tableSteps }: { step: number; tableSteps: ReturnType<typeof generateTableSteps>["steps"] }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      ...
      <div className="text-center">
        ...
        <div className="flex gap-1 justify-center">
          {currentDp.map((len, dpIndex) => (
              <div key={`dp-${len}-${dpIndex}`} className="flex flex-col items-center">
      ...
      {currentStep && (
        <div className={`text-sm text-center font-mono px-6 py-3 rounded-lg ${currentStep.improved ? "bg-green-600/20 text-green-400" : "bg-gray-800/50 text-gray-400"}`}>
          {currentStep.formula}
        </div>
      )}
      <div className="text-sm text-gray-500">For each i, check all j &lt; i where nums[j] &lt; nums[i]</div>
    </div>
  );
};

const BinaryPhase = ({ step, binarySteps }: { step: number; binarySteps: ReturnType<typeof generateBinarySteps>["steps"] }) => {
  const currentStep = step > 0 && step <= binarySteps.length ? binarySteps[step - 1] : null;
      ...
      <div className="text-center">
        ...
        <div className="flex gap-1 justify-center min-h-[50px]">
          {currentTails.length > 0 ? currentTails.map((val, tailIndex) => (
              <div key={`tail-${val}-${tailIndex}`} className="flex flex-col items-center">
  ...
  return (
    <div className="p-6 bg-gray-900 rounded-xl w-full max-w-4xl mx-auto">
      <div className="text-center mb-4">
        <div className="text-lg font-medium text-white">Longest Increasing Subsequence (LIS)</div>
        <div className="text-sm text-gray-400">Find the length of the longest strictly increasing subsequence</div>
      </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={currentPhase} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          {currentPhase === "binary" && <BinaryPhase step={step} binarySteps={binarySteps} />}
        </motion.div>
      </AnimatePresence>
      <div className="mt-6 pt-4 border-t border-gray-800 text-sm text-gray-500 text-center">
        nums = [{nums.join(", ")}] | LIS length = {maxLen} | Example: [2, 3, 7, 101]
      </div>
    </div>
  );
}
