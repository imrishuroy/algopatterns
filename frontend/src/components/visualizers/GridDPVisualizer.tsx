...
const generatePathSteps = () => {
  ...
  for (let c = 1; c < cols; c++) {
    dp[0][c] = 1;
    steps.push({ r: 0, c, value: 1, formula: `dp[0][${c}] = 1 (only from left)` });
  }

  for (let r = 1; r < rows; r++) {
    dp[r][0] = 1;
    steps.push({ r, c: 0, value: 1, formula: `dp[${r}][0] = 1 (only from above)` });
  }

  for (let r = 1; r < rows; r++) {
    for (let c = 1; c < cols; c++) {
      dp[r][c] = dp[r - 1][c] + dp[r][c - 1];
      steps.push({ r, c, value: dp[r][c], formula: `dp[${r}][${c}] = dp[${r - 1}][${c}] + dp[${r}][${c - 1}] = ${dp[r - 1][c]} + ${dp[r][c - 1]} = ${dp[r][c]}` });
    }
  }
...
const generateMinPathSteps = () => {
  ...
  dp[0][0] = grid[0][0];
  steps.push({ r: 0, c: 0, value: dp[0][0], formula: `dp[0][0] = grid[0][0] = ${grid[0][0]}` });

  for (let c = 1; c < cols; c++) {
    dp[0][c] = dp[0][c - 1] + grid[0][c];
    steps.push({ r: 0, c, value: dp[0][c], formula: `dp[0][${c}] = ${dp[0][c - 1]} + ${grid[0][c]} = ${dp[0][c]}` });
  }

  for (let r = 1; r < rows; r++) {
    dp[r][0] = dp[r - 1][0] + grid[r][0];
    steps.push({ r, c: 0, value: dp[r][0], formula: `dp[${r}][0] = ${dp[r - 1][0]} + ${grid[r][0]} = ${dp[r][0]}` });
  }

  for (let r = 1; r < rows; r++) {
    for (let c = 1; c < cols; c++) {
      dp[r][c] = Math.min(dp[r - 1][c], dp[r][c - 1]) + grid[r][c];
      steps.push({ r, c, value: dp[r][c], formula: `dp[${r}][${c}] = min(${dp[r - 1][c]}, ${dp[r][c - 1]}) + ${grid[r][c]} = ${dp[r][c]}` });
    }
  }
...
const GridDisplay = ({ dpTable, currentCell, showGrid, showCost }: { dpTable: number[][]; currentCell?: { r: number; c: number }; showGrid: boolean; showCost: boolean }) => (
  <div className="flex justify-center gap-8">
    ...
    <div className="text-center">
      <div className="text-sm text-gray-500 mb-2">{showCost ? "Min Path Sum" : "Unique Paths"}</div>
      <div className="inline-grid gap-1" style={{ gridTemplateColumns: `repeat(${cols}, 1fr)` }}>
...
const PathsPhase = ({ step, pathSteps }: { step: number; pathSteps: ReturnType<typeof generatePathSteps>["steps"] }) => {
  ...
  const currentStep = step > 0 && step <= pathSteps.length ? pathSteps[step - 1] : null;

  return (
    <div className="flex flex-col items-center gap-6">
      <GridDisplay dpTable={dpTable} currentCell={currentStep ? { r: currentStep.r, c: currentStep.c } : undefined} showGrid={false} showCost={false} />
      {currentStep && (
        <div className="text-sm text-center font-mono bg-gray-800/50 text-gray-400 px-6 py-3 rounded-lg">{currentStep.formula}</div>
      )}
      <div className="text-sm text-gray-500">dp[r][c] = dp[r-1][c] + dp[r][c-1] (paths from above + left)</div>
    </div>
  );
};

const MinPathPhase = ({ step, minPathSteps }: { step: number; minPathSteps: ReturnType<typeof generateMinPathSteps>["steps"] }) => {
  const dpTable = useMemo(() => {
    const dp: number[][] = Array(rows).fill(null).map(() => Array(cols).fill(0));
  ...
  const currentStep = step > 0 && step <= minPathSteps.length ? minPathSteps[step - 1] : null;
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      ...
      <div className="text-sm text-gray-500">dp[r][c] = min(dp[r-1][c], dp[r][c-1]) + grid[r][c]</div>
    </div>
  );
};
...
export default function GridDPVisualizer() {
  ...
  const { steps: pathSteps, dp: pathDp } = useMemo(() => generatePathSteps(), []);
  const { steps: minPathSteps, dp: minPathDp } = useMemo(() => generateMinPathSteps(), []);
  ...
  return (
    <div className="p-6 bg-gray-900 rounded-xl w-full max-w-4xl mx-auto">
      <div className="text-center mb-4">
        <div className="text-lg font-medium text-white">Grid DP (2D Navigation)</div>
        <div className="text-sm text-gray-400">Count paths or find min cost from top-left to bottom-right</div>
      </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={currentPhase} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          {currentPhase === "paths" && <PathsPhase step={step} pathSteps={pathSteps} />}
          {currentPhase === "minpath" && <MinPathPhase step={step} minPathSteps={minPathSteps} />}
          {currentPhase === "table" && <MinPathPhase step={step} minPathSteps={minPathSteps} />}
        </motion.div>
      </AnimatePresence>
      <div className="mt-6 pt-4 border-t border-gray-800 text-sm text-gray-500 text-center">
        {rows}x{cols} grid | Unique Paths = {pathDp[rows - 1][cols - 1]} | Min Path Sum = {minPathDp[rows - 1][cols - 1]}
      </div>
    </div>
  );
}
