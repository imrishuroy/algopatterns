...
const generateTableSteps = () => {
  ...
  for (let i = 0; i < n; i++) {
    ...
    steps.push({
      ...
      j: i,
      isPalin: true,
      formula: `Single char '${str[i]}' is palindrome`,
      len: 1,
    });
  }
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    steps.push({
      ...
      j: i + 1,
      isPalin,
      formula: `'${str[i]}${str[i + 1]}': ${str[i]} ${isPalin ? "==" : "!="} ${str[i + 1]}`,
      len: 2,
    });
  }
  ...
  return {
    steps,
    dp,
    longest: str.substring(longestStart, longestStart + longestLen),
  };
};
...
const StringDisplay = ({
  ...
  highlightRange?: { left: number; right: number };
}) => (
  <div className="flex justify-center gap-1 mb-4">
    {str.split("").map((c, i) => {
      const inRange =
        highlightRange && i >= highlightRange.left && i <= highlightRange.right;
      return (
        <div key={`str-char-${i}-${c}`} className="flex flex-col items-center">
          <div className="text-xs text-gray-500 mb-1">{i}</div>
          <div
            className={`w-10 h-10 flex items-center justify-center rounded font-mono font-bold text-lg transition-all ${
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      <StringDisplay
        highlightRange={
          currentStep
            ? { left: currentStep.i, right: currentStep.j }
            : undefined
        }
      />
            ...
            <tbody>
              {str.split("").map((c, i) => (
                <tr key={`tr-row-${i}-${c}`}>
                  ...
                  {str.split("").map((_, j) => {
                    const isCurrent =
                      currentStep && currentStep.i === i && currentStep.j === j;
                        ...
                        <div
                          className={`w-10 h-10 flex items-center justify-center border-2 rounded font-mono text-xs ${
                            !isValid
                              ...
                              : isCurrent
                                ? `${value ? "bg-green-600 border-green-400" : "bg-red-600/50 border-red-400"} text-white font-bold`
                                : value === true
      ...
      {currentStep && (
        <div className="text-sm text-center">
          ...
          <div
            className={`font-mono px-6 py-3 rounded-lg ${currentStep.isPalin ? "bg-green-600/20 text-green-400" : "bg-gray-800/50 text-gray-400"}`}
          >
            {currentStep.formula}
          </div>
        </div>
      ...
      <div className="text-sm text-gray-500">
        Process by LENGTH: len=1, then len=2, then len=3...
      </div>
    </div>
  );
};
...
const ExpandPhase = ({
  ...
  step: number;
  expandSteps: ReturnType<typeof generateExpandSteps>["steps"];
}) => {
  const currentStep =
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      <StringDisplay
        highlightRange={
          currentStep
            ? { left: currentStep.left, right: currentStep.right }
            : undefined
        }
      />
      ...
      <div className="text-center">
        {currentStep && (
          <>
            <div className="text-sm text-gray-400 mb-2">
              ...
              {currentStep.type === "odd" ? "single" : "between"})
            </div>
            <div className="text-2xl font-mono text-purple-400 mb-2">
        ...
        {!currentStep && (
          <div className="text-gray-500">
            Press play to expand around centers
          </div>
        )}
      </div>

      <div className="text-sm text-gray-500 text-center max-w-md">
        For each center, expand while s[left] == s[right]. Check both odd-length
        (single center) and even-length (two centers).
      </div>
    </div>
  );
};
